...
from dimos.mapping.osm.distance import distance_in_meters
...
class OsmSkillContainer(SkillContainer):
    ...
    def __enter__(self) -> "OsmSkillContainer":
        self._disposables.add(self._position_stream.subscribe(self._on_position))
        return self
    ...
    def _on_position(self, position: LatLon) -> None:
    ...
    def street_map_query(self, query_sentence: str) -> str:
        ...
        self._current_location_map.update_position(self._latest_position)
        location = self._current_location_map.query_for_one_position_and_context(
            query_sentence, self._latest_position
        )
        if not location:
        ...
        distance = int(distance_in_meters(latlon, self._latest_position))

...
class FastAPIServer(EdgeIO):
    def __init__(self, dev_name="FastAPI Server", edge_type="Bidirectional", host="0.0.0.0", port=5555, **streams):
        ...
        for key in self.streams:
            if self.streams[key] is not None:
                self.active_streams[key] = self.streams[key].pipe(
                    ops.map(self.process_frame_fastapi),
                    ops.share()
                )
    ...
    def setup_routes(self):
        ...
        async def index(request: Request):
            ...
            return self.templates.TemplateResponse("index_fastapi.html", {"request": request, "stream_keys": stream_keys})

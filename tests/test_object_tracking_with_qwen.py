...
tracker_stream = ObjectTrackingStream(
    camera_intrinsics=camera_intrinsics, camera_pitch=0.0, camera_height=0.5
)
...
if "DISPLAY" not in os.environ:
    raise RuntimeError(
        "No display available. Please set DISPLAY environment variable or run in headless mode."
    )
...
def on_next(result):
    ...
    cv2.putText(
        viz_frame,
        f"Tracking {tracking_object_name}",
        (10, 30),
    ...
    cv2.putText(
        viz_frame,
        f"Object size: {object_size:.2f}m",
        (10, 60),
        cv2.FONT_HERSHEY_SIMPLEX,
        0.7,
        (255, 255, 255),
    ...
    cv2.putText(viz_frame, f"Status: {status}", (10, 90), cv2.FONT_HERSHEY_SIMPLEX, 0.7, color, 2)

    # If detection is in progress, show a message
    if detection_in_progress:
        cv2.putText(
            viz_frame, "Querying Qwen...", (10, 120), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2
        )
